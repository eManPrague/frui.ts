...
export interface HasLifecycleEvents<TScreen = unknown, TNavigationParams = unknown, TLocation = unknown> {
  ...
  onInitialize: (context: NavigationContext<TScreen, TNavigationParams, TLocation>) => Awaitable<unknown>;
  ...
  canNavigate: (context: NavigationContext<TScreen, TNavigationParams, TLocation>) => Awaitable<boolean>;

  /** Called whenever navigate is called on a deactivated screen */
  onActivate: (context: NavigationContext<TScreen, TNavigationParams, TLocation>) => Awaitable<unknown>;

  /** Called on every navigattion path change */
  onNavigate: (context: NavigationContext<TScreen, TNavigationParams, TLocation>) => Awaitable<unknown>;
