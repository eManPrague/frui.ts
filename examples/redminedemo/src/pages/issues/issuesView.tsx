...
const Filter: React.FunctionComponent<{ vm: IssuesViewModel }> = observer(
  ({ vm }) =>
    vm.filter && (
      <fieldset>
        ...
        <div className="form-row">
          ...
          <div className="col">
            <FormField label="Project" target={vm.filter} property="project_id">
              {(bindingProps, childProps) => (
                <DropDown {...bindingProps} {...childProps} options={vm.projects} allowEmpty={true} />
              )}
            </FormField>
          </div>
          <div className="col">
        ...
        <div className="float-right">
          <button className="btn btn-secondary mb-4" onClick={vm.resetFilterAndLoad}>
            Reset
          </button>{" "}
          &nbsp;
          <button className="btn btn-primary mb-4" onClick={vm.applyFilterAndLoad}>
            Load
          </button>
        </div>
      </fieldset>
  ...
  !vm.items ? null : (
    <table className="table table-hover">
      <thead className="thead-light">
        <tr>
          <SortingHeader column="id" filter={vm.pagingFilter} onSortChanged={vm.loadData}>
            ID
          </SortingHeader>
          <SortingHeader column="project" filter={vm.pagingFilter} onSortChanged={vm.loadData}>
            Project
          </SortingHeader>
          <SortingHeader column="subject" filter={vm.pagingFilter} onSortChanged={vm.loadData}>
            Subject
          </SortingHeader>
          <th>&nbsp;</th>
        </tr>
      ...
      <tbody>
        {vm.items.map(issue => (
          <tr key={issue.id}>
            ...
            <td>{issue.subject}</td>
            <td>
              <button className="btn btn-link p-0" data-id={issue.id} onClick={createDataIdHandler(vm.openDetail)}>
                Open
              </button>{" "}
            </td>
          </tr>
